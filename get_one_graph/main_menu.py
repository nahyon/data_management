...
class MainMenu(QMainWindow, MainMenuUI):
    ...
    def show_graph(self) : #그래프 보여주는 함수
        ...
        ax.plot(self.x1, self.y1, label="2022", color='red', linewidth=1.5)
        ax.plot(self.x2, self.y2, label="2021", color='green', linewidth=1.5)
        #ax.grid(True)
        
        ax.set_ylim([0, 100]) #y축 0~100으로고정
        ax.set_xticks(list((range(monitor_start_station, monitor_end_station+20, 100)))) #눈금
    ...
    def png_save(self) :
        ...
        plt.plot(self.x1, self.y1, label="2022", color='red', linewidth=1.5)
        plt.plot(self.x2, self.y2, label="2021", color='green', linewidth=1.5)
        #ax.grid(True)
